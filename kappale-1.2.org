...
** Vihjeitä
   ...
   - Funktion \(g(n)\) arvoa laskettaessa sijoitusmallilla voidaan
     toistuvasti käyttää funktion \(f\) lauseketta.
   - Vastaavasti funktion \(h(n)\) arvoa laskettaessa sijoitusmallilla
     voidaan toistuvasti käyttää funktion \(g\) lauseketta.
** Ratkaisu
   #+BEGIN_SRC scheme :exports both
     (define (A x y)
       (cond ((= y 0) 0)
   ...
   &= g (A(2, n-1))\\
   &= 2^{A(2, n-1)}\\
   &= 2^{2^{A(2,n-2)}} = \cdots = \underbrace{2^{2^{{\cdot}^{{\cdot}^{{\cdot}^2}}}}}_{\text{$n$ kpl}}.
   \end{align*}
